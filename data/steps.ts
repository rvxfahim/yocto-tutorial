...
export const steps: StepDefinition[] = [
...

  {
    ...
    content: {
      ...
      objective:
        "Create the top-level project directory structure: `~/my_project/` with `sources/`, `downloads/`, and `sstate-cache/` subdirectories.",
  ...
  // ==================== PHASE 4: Custom Layer & Kernel Module ====================
  {
    ...
    content: {
      ...
      objective:
        "Create the `meta-my_project` layer with the standard Yocto layer directory layout and write `conf/layer.conf`.",
      why: "Custom layers are how you extend Yocto without modifying upstream code. The directory structure follows a convention:\n\n• `conf/layer.conf` — Layer metadata (name, priority, dependencies, recipe search paths)\n• `recipes-kernel/` — Kernel-related recipes and config fragments\n• `recipes-apps/` — Application recipes\n• `recipes-core/` — Core system recipes (images, base packages)\n\nThe layer.conf tells BitBake: 'Here are my recipes, here's my priority, and I depend on the core layer.' BBFILE_PRIORITY of 5 means our layer overrides upstream layers with lower priority.\n\nWe also create a `.bbappend` file (`linux-yocto_%.bbappend`) — this extends the kernel recipe from poky without modifying the original. The `%` wildcard matches any version of `linux-yocto`. It adds a kernel config fragment (`iio.cfg`) that enables the IIO subsystem, which our mock-adxl345 driver requires. These are **manually created** files — they are not auto-generated by any Yocto tool.",
      tips: [
      ...
      glossaryTerms: [
        ...
        { term: "BBFILES", definition: "See glossary" },
        { term: "BBPATH", definition: "See glossary" },
        { term: "bbappend", definition: "See glossary" },
        { term: "Kernel Config Fragment", definition: "See glossary" },
      ],
    },
    terminal: { prompt: "$ ", lines: [] },
    files: [],
  ...
  // ==================== PHASE 6: Image & Build ====================
  {
    ...
    content: {
      ...
      objective:
        "Write `my-project-image.bb` — the image recipe that assembles the final bootable Linux system by pulling together the kernel, our modules, and the user-space application.",
...

  {
    ...
    content: {
      ...
      objective:
        "Run `bitbake my-project-image` to build the complete bootable image. This is the big one — the first build takes 2-4 hours.",
